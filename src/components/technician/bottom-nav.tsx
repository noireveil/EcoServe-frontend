...
const navItems = [
  { href: "/technician/orders", icon: ClipboardList, label: "Orders" },
  { href: "/technician/map", icon: Map, label: "Map" },
  { href: "/technician/earnings", icon: Wallet, label: "Earnings" },
...
function DesktopTopNav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 hidden h-16 items-center justify-between border-b border-border/50 bg-card/95 backdrop-blur-lg px-6 md:flex">
      {/* Logo */}
      <Link href="/technician/dashboard" className="flex items-center gap-2 text-foreground hover:text-primary transition-colors">
      ...
      <Link
        ...
        className={cn(
          ...
          isActive("/technician/profile") && "border-primary/50 bg-primary/10"
        )}
      >
